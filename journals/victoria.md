...
* .gitlab-ci.yml
    ...
    I was finally able to get my lint test to run! I ran flake8 and fixed all of the errors in both of our microservices and got the pipeline to run successfully! I am now trying to get the monolith back end test running but have run into a few errors. First I needed to adjust our settings file aws keys to get methods because it was not pulling the information from .env, I saw this was a common error online and was able to resolve the issue by changing them. My current blocker is I am stuck on an error for settings.databases, it is telling me it needs me to define engine, however I have already set the information as defined in django settings documentation.
